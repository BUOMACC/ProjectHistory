...
### 1.2 업무 내용
- 다수의 자식을 가질 수 있는 트리 구조 기반 스킬자원 관리 시스템 설계
  → 스킬 커스터마이징 / 테이블을 통한 기능확장 제공
...
- 대량의 테이블 데이터를 가지면서 휴먼에러로 인한 긴 테스트 시간 발생

   → Editor Utility Widget 기반 데이터 검증 툴을 개발하여 휴먼에러로 인한 디버깅 시간을 기존 30분~1시간 수준에서 10분 이내로 단축


<a name="bp-to-cpp"></a>
## 5. Blueprint -> C++ 포팅
- Blueprint 수정 / 로드하는 상황에서 분 단위로 지연되는 문제 발생
- Blueprint 간 순환 참조로 인한 의존성이 원인임을 파악 (에셋로드 + 재컴파일)
- 핵심 로직을 C++로 이전하고, 필요한 경우 Soft Reference로 관리 → Blueprint 로드시간 2배 이상 단축


<a name="reconnect"></a>
## 6. Reconnect
- Inactive 상태의 PlayerState 기반 플레이어 재연결 구조 설계
- 플레이어 연결 해제시 발생하는 Pawn Destroy / Movement 비정상 동작 문제 해결
